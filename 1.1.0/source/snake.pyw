...
class SnakeGame(QMainWindow):
    def __init__(self):
        ...
        self.lives_label_text = QLabel("Lives Remaining:")
    ...
    def update_lives_visibility(self):
        ...
        if infinite:
            ...
            self.lives_label_text.setText(f"Lives Remaining: {self.current_lives}")
        self.update_ui()
    ...
    def ai_move(self):
        ...
        fx, fy = self.food
        hx, hy = head
        dx = (fx - hx + self.grid_width) % self.grid_width
        if dx > self.grid_width // 2:
            dx -= self.grid_width
        dy = (fy - hy + self.grid_height) % self.grid_height
        if dy > self.grid_height // 2:
            dy -= self.grid_height
        directions = []
        if abs(dx) >= abs(dy):
            if dx > 0:
                directions.append(Qt.Key.Key_Right)
            elif dx < 0:
                directions.append(Qt.Key.Key_Left)
            if dy > 0:
                directions.append(Qt.Key.Key_Down)
            elif dy < 0:
                directions.append(Qt.Key.Key_Up)
        else:
            if dy > 0:
                directions.append(Qt.Key.Key_Down)
            elif dy < 0:
